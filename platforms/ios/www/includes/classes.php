...
function menu($user) {
	...
	if($user !== false) {
		...
		// Else show the Admin button
		return '<a href="'.$CONF['url'].'/index.php?a=admin"><div class="menu_btn" title="'.$LNG['admin_login'].'"><img src="'.$CONF['url'].'/'.$CONF['theme_url'].'/images/register.png" /></div></a>';
	}
}
...
class feed {
	...
	function getSubscribe($type = null, $list = null, $z = null) {
		...
		if(!empty($this->username) && $this->username !== $profile['username']) {
			...
			return '<a href="'.$this->url.'/index.php?a=settings" title="'.$LNG['admin_desc_edit_users'].'"><div class="edit_profile_btn'.$style.'"></div></a>';
		} else {
			return false;
		}
